import { Text } from 'native-base'

export default function ChecklistPage() {
  return <Text>Tela Nova Ação</Text>
}
// import { zodResolver } from '@hookform/resolvers/zod'
// import { router, useLocalSearchParams } from 'expo-router'
// import { useToast } from 'native-base'
// import { useEffect, useState } from 'react'
// import { FormProvider, useForm } from 'react-hook-form'
// import { Alert, BackHandler } from 'react-native'
// import { z } from 'zod'
// import { Button } from '../../src/components/Button'
// import { Form } from '../../src/components/Form'
// import { KeyboardCoverPrevent } from '../../src/components/KeyboradCoverPrevent'
// import { Loading } from '../../src/components/Loading'
// import { Toast } from '../../src/components/Toast'
// import { useAuth } from '../../src/hooks/useAuth'
// import { IChecklistPeriod } from '../../src/libs/database/@types/IChecklistPeriod'
// import { useChecklist } from '../../src/store/checklist'
// import { useResponsibles } from '../../src/store/responsibles'
// import {
//   Buttons,
//   Container,
//   FormContainer,
//   Header,
//   SubTitle,
//   Title,
// } from './styles'

// const newActionSchema = z.object({
//   title: z
//     .string({ required_error: 'Escreva o nome da ação ' })
//     .nonempty({ message: 'Escreva o nome da ação' }),
//   responsible: z
//     .string({ required_error: 'Selecione um responsável' })
//     .nonempty({ message: 'Selecione um responsável' }),
//   dueDate: z.date(),
// })

// type NewActionData = z.infer<typeof newActionSchema>

// export default function NewAction() {
//   const newActionForm = useForm<NewActionData>({
//     resolver: zodResolver(newActionSchema),
//   })
//   const { handleSubmit, reset } = newActionForm
//   const {
//     createNewAction,
//     load,
//     loadActions,
//     setCurrentAction,
//     currentChecklist,
//     updateAnswering,
//   } = useChecklist()
//   const { responsibles, fetchResponsibles } = useResponsibles()
//   const { checklistPeriodId } = useLocalSearchParams()
//   const { user, token } = useAuth()
//   const toast = useToast()
//   const [isLoading, setIsLoading] = useState(false)
//   const [currentPeriod, setCurrentPeriod] = useState<IChecklistPeriod | null>(
//     null,
//   )

//   function handleStop() {
//     Alert.alert('Sair', 'Deseja abandonar essa ação?', [
//       {
//         text: 'Não',
//         style: 'cancel',
//       },
//       {
//         text: 'Sim',
//         style: 'destructive',
//         onPress: () => {
//           updateAnswering(false)
//           router.replace('/edit-checklist')
//         },
//       },
//     ])
//     return true
//   }

//   async function handleNewAction(data: NewActionData) {
//     try {
//       setIsLoading(true)
//       const action = await createNewAction({
//         checklistId: currentChecklist.id,
//         actionProps: {
//           description: '',
//           dueDate: data.dueDate.toISOString(),
//           responsible: data.responsible,
//           endDate: '',
//           title: data.title,
//           startDate: new Date().toISOString(),
//         },
//         checklistPeriodId: Number(checklistPeriodId),
//         user: user.login,
//       })
//       setCurrentAction(action.id)
//       reset()
//       load(user.login)
//       loadActions(user.login)
//       setIsLoading(false)
//       updateAnswering(false)
//       toast.show({
//         render: () => (
//           <Toast.Success>Ação registrada com sucesso!</Toast.Success>
//         ),
//       })
//       router.replace('/edit-checklist')
//     } catch (err) {
//       console.log(err)
//       toast.show({
//         render: () => <Toast.Error>Erro ao registrar ação!</Toast.Error>,
//       })
//     }
//   }

//   useEffect(() => {
//     const backHandler = BackHandler.addEventListener(
//       'hardwareBackPress',
//       handleStop,
//     )

//     return () => backHandler.remove()
//   }, [])

//   useEffect(() => {
//     if (currentChecklist) {
//       const period = currentChecklist.checklistPeriods.find(
//         (item) => item.id === Number(checklistPeriodId),
//       )
//       setCurrentPeriod(period)
//     }
//   }, [currentChecklist, checklistPeriodId])

//   if (!responsibles || !responsibles?.length || !currentPeriod) {
//     fetchResponsibles(user.login, token)
//     return (
//       <Container>
//         <Loading />
//       </Container>
//     )
//   }

//   return (
//     <KeyboardCoverPrevent>
//       <Container>
//         <Header>
//           <Title>Registrar nova Ação</Title>
//           <SubTitle>
//             {currentPeriod.task.description} - {currentPeriod.task.answer}
//           </SubTitle>
//         </Header>

//         <FormProvider {...newActionForm}>
//           <FormContainer>
//             <Form.Field>
//               <Form.Label>Ação:</Form.Label>
//               <Form.Input name="title" />
//               <Form.ErrorMessage field="title" />
//             </Form.Field>
//             <Form.Field>
//               <Form.Label>Responsável:</Form.Label>
//               <Form.SelectFlash
//                 name="responsible"
//                 options={responsibles.map((responsible) => ({
//                   label: responsible.name,
//                   value: responsible.login,
//                 }))}
//               />
//               <Form.ErrorMessage field="responsible" />
//             </Form.Field>
//             <Form.Field>
//               <Form.Label>Prazo:</Form.Label>
//               <Form.DatePicker
//                 placehilderFormat="DD/MM/YYYY"
//                 name="dueDate"
//                 mode="datetime"
//               />
//             </Form.Field>
//           </FormContainer>
//           <Buttons>
//             <Button.Trigger
//               onPress={handleStop}
//               variant="secondary"
//               style={{ width: '49%' }}
//             >
//               <Button.Text>Cancelar</Button.Text>
//             </Button.Trigger>
//             <Button.Trigger
//               variant="green"
//               style={{ width: '49%' }}
//               loading={isLoading}
//               onPress={handleSubmit(handleNewAction)}
//             >
//               <Button.Text>Confirmar</Button.Text>
//               <Button.Icon.CheckCircle />
//             </Button.Trigger>
//           </Buttons>
//         </FormProvider>
//       </Container>
//     </KeyboardCoverPrevent>
//   )
// }
